...
Authors: David Mutchler, Dave Fisher, Valerie Galluzzi, Amanda Stouder,
         their colleagues and Daniel Johnson.  September 2016.
...
class Point(object):
    ...
    def move_to(self, x, y):
        x0 = self.x
        y0 = self.y
        self.distance = self.distance + ((x-x0)**2+(y-y0)**2)**0.5
        self.x = x
        self.y = y
    ...
    def get_distance_from_start(self):
        return ((self.x-self.startx)**2+(self.y-self.starty)**2)**0.5

    def get_distance_traveled(self):
        return self.distance

    def closer_to(self, p2, p3):
        d2 = ((self.x-p2.x)**2+(self.y-p2.y)**2)**0.5
        d3 = ((self.x - p3.x) ** 2 + (self.y - p3.y) ** 2) ** 0.5
        if(d3 < d2):
            return p3
        else:
            return p2
...
def run_test_get_number_of_moves_made():
    ...
    Side effects:
       ** You figure out what side effect(s) MUST happen! **
       Point must have an instance variable 'moves' that increments by 1 every time move_to or move_by is called
...
def run_test_get_distance_from_start():
    ...
    Side effects:
       ** You figure out WHETHER OR NOT side effect(s) MUST happen! **
       Two instances variables startx and starty must be defined and set to the initial position of the point's x and y
       coordinates
...
def run_test_get_distance_traveled():
    ...
    Side effects:
       ** You figure out WHETHER OR NOT side effect(s) MUST happen! **
       a variable 'distance' will need to be implemented in move_to and move_by to record the total distance traveled
